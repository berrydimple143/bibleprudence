<div class="p-5">
    @if($showForm == 'add')
    <livewire:admin.create-book></livewire:admin.create-book>
    @elseif($showForm == 'edit')
    <livewire:admin.update-book :book="$book"></livewire:admin.update-book>
    @else
    <div class="w-full">
        <div class="w-full bg-gray-800 border-b-2 border-dotted border-gray-600 p-4 flex items-center justify-between">
            <div class="flex gap-3">
                <div class="flex gap-2 items-center">
                    ...
                    <span class="text-xl text-white">Books Table</span>
                </div>
                <p class="text-xl text-white flex items-center gap-4">
                    <span>
                        <button wire:click="addBook" wire:loading.class="opacity-50"
            ...
            <input type="text" class="px-4 py-1 rounded-full border border-white shadow-lg"
                wire:model.live.debounce.300ms="search" placeholder="Search book here ...">
        </div>
            ...
            <table class="min-w-full bg-white">
                <thead class="bg-gray-800 text-white">
                    <tr>
                        ...
                        <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm"><span class="cursor-pointer" wire:click="sort('name')">Book Name</span></th>
                ...
                <tbody class="text-gray-700">
                    ...
                    <?php
                        $index = ($books ->currentpage()-1) * $books ->perpage() + $loop->index + 1;
                        $addClass = '';
                        if($index % 2 === 0) {
                            $addClass = 'bg-gray-200';
                        }
                    ?>
                    <tr class="{{ $addClass }}" wire:key="{{ $book->id }}">
                        <td class="w-5 text-left py-3 px-4 font-semibold text-sm">{{ $index }}</td>
                        <td class="w-1/3 text-left py-3 px-4">{{ $book->name }}</td>
                        <td class="text-left py-3 px-4 flex gap-2 text-white items-center justify-center">
                            ...
                            <button wire:confirm="Are you sure you want to delete this book?"
                                wire:click="deleteBook({{ $book->id }})" type="button"
