<div class="p-5">
    @if($showForm == 'add')
    <livewire:admin.create-comment></livewire:admin.create-comment>
    @elseif($showForm == 'edit')
    <livewire:admin.update-comment :comment="$comment"></livewire:admin.update-comment>
    @else
    <div class="w-full">
        <div class="w-full bg-gray-800 border-b-2 border-dotted border-gray-600 p-4 flex items-center justify-between">
            <div class="flex gap-3">
                <div class="flex gap-2 items-center">
                    ...
                    <span class="text-xl text-white">Comments Table</span>
                </div>
                <p class="text-xl text-white flex items-center gap-4">                
                    <span>
                        <button wire:click="addComment" wire:loading.class="opacity-50"
            ...
            <input type="text" class="px-4 py-1 rounded-full border border-white shadow-lg"
                wire:model.live.debounce.300ms="search" placeholder="Search comment here ...">
        </div>
            ...
            <table class="min-w-full bg-white">
                <thead class="bg-gray-800 text-white">
                    <tr>
                        ...
                        <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm"><span class="cursor-pointer" wire:click="sort('author')">Author</span></th>
                        <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm">Quiz Question</th>
                        <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm"><span class="cursor-pointer" wire:click="sort('created_at')">Date Commented</span></th>
                ...
                <tbody class="text-gray-700">
                    ...
                    <?php
                        $index = ($comments ->currentpage()-1) * $comments ->perpage() + $loop->index + 1;
                        $addClass = '';
                        if($index % 2 === 0) {
                            $addClass = 'bg-gray-200';
                        }
                        $dt = formatDate($comment->created_at);
                    ?>
                    <tr class="{{ $addClass }}" wire:key="{{ $comment->id }}">
                        <td class="w-5 text-left py-3 px-4 font-semibold text-sm">{{ $index }}</td>
                        <td class="w-1/3 text-left py-3 px-4">{{ $comment->message }}</td>
                        <td class="w-1/3 text-left py-3 px-4">{{ $comment->author }}</td>
                        <td class="w-1/3 text-left py-3 px-4">{{ $comment->quiz->question }}</td>
                        <td class="w-1/3 text-left py-3 px-4">{{ $dt }}</td>
                        <td class="text-left py-3 px-4 flex gap-2 text-white">
                            <button wire:click="changeStatus('{{ $comment->id }}', '{{ $comment->status }}')" type="button"
                                class="@if($comment->status == 1) bg-green-500 @else bg-gray-500 @endif px-4 py-2 rounded-sm shadow-md flex items-center gap-1 hover:bg-yellow-700">
