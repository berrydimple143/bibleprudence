<div class="p-5">
    @if($showForm == 'add')
    <livewire:admin.create-math-option></livewire:admin.create-math-option>
    @elseif($showForm == 'edit')
    <livewire:admin.update-math-option :option="$option"></livewire:admin.update-math-option>
    @else
    <div class="w-full">
        <div class="w-full bg-gray-800 border-b-2 border-dotted border-gray-600 p-4 flex items-center justify-between">
            <div class="flex gap-3">
                <div class="flex gap-2 items-center">
                    ...
                    <span class="text-xl text-white">Math Choices</span>
                </div>
                <p class="text-xl text-white flex items-center gap-4">
                    <span>
                        <button wire:click="addOption" wire:loading.class="opacity-50"
            ...
            <table class="min-w-full bg-white">
                <thead class="bg-gray-800 text-white">
                    <tr>
                        ...
                        <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm">Math Question</th>
                ...
                <tbody class="text-gray-700">
                    ...
                    <tr class="{{ $addClass }}" wire:key="{{ $choice->id }}">
                        ...
                        <td class="w-1/3 text-left py-3 px-4">{{ $choice->bmath->question }}</td>
                        <td class="w-1/3 text-left py-3 px-4">{{ $choice->option }}</td>
                        <td class="text-left py-3 px-4 flex gap-2 text-white">
                            <button wire:click="editOption('{{ $choice->id }}')" type="button"
                            ...
                            <button wire:confirm="Are you sure you want to delete this choice?"
                                wire:click="deleteOption({{ $choice->id }})" type="button"
