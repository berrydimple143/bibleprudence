<div class="p-5">
    @if($showForm == 'add')
    <livewire:admin.create-point></livewire:admin.create-point>
    @elseif($showForm == 'edit')
    <livewire:admin.update-point :point="$point"></livewire:admin.update-point>
    @else
    <div class="w-full">
        <div class="w-full bg-gray-800 border-b-2 border-dotted border-gray-600 p-4 flex items-center justify-between">
            <div class="flex gap-3">
                <div class="flex gap-2 items-center">
                    ...
                    <span class="text-xl text-white">Outline Main Points Table</span>
                </div>
                <p class="text-xl text-white flex items-center gap-4">
                    <span>
                        <button wire:click="addPoint" wire:loading.class="opacity-50"
            ...
            <input type="text" class="px-4 py-1 rounded-full border border-white shadow-lg"
                wire:model.live.debounce.300ms="search" placeholder="Search main point here ...">
        </div>
        <div class="bg-white overflow-auto">
            <table class="min-w-full bg-white">
                <thead class="bg-gray-800 text-white">
                    <tr> 
                        ...
                        <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm">
                            <span class="cursor-pointer" wire:click="sort('main')">Main Points</span>          
                        </th>
                        <th class="w-96 text-left py-3 px-4 uppercase font-semibold text-sm">
                            <span class="cursor-pointer" wire:click="sort('verse')">Verse</span>               
                        </th>
                        <th class="w-96 text-left py-3 px-4 uppercase font-semibold text-sm">Text Passage</th>
                        <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm">Theme</th>
                ...
                <tbody class="text-gray-700">
                    ...
                    <?php
                        $index = ($points ->currentpage()-1) * $points ->perpage() + $loop->index + 1;
                        $addClass = '';
                        if($index % 2 === 0) {
                            $addClass = 'bg-gray-200';
                        }
                    ?>
                    <tr class="{{ $addClass }}" wire:key="{{ $point->id }}">
                        <td class="text-left py-3 px-4 font-semibold text-sm">{{ $index }}</td>
                        <td class="text-left py-3 px-4">{{ $point->main }}</td>
                        <td class="text-left py-3 px-4">{{ $point->verse }}</td>
                        <td class="text-left py-3 px-4">{{ $point->outline->text }}</td>
                        <td class="text-left py-3 px-4">{{ $point->outline->theme }}</td>
                        <td class="text-left py-3 px-4 flex gap-2 text-white">
                            <button wire:click="editPoint('{{ $point->id }}')" type="button"
                            ...
                            <button wire:confirm="Are you sure you want to delete this main point?"
                                wire:click="deletePoint({{ $point->id }})" type="button"
