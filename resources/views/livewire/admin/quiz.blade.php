<div class="p-5">
    @if($showForm == 'add')
    <livewire:admin.create-quiz></livewire:admin.create-quiz>
    @elseif($showForm == 'edit')
    <livewire:admin.update-quiz :quiz="$quiz"></livewire:admin.update-quiz>
    @else
    <div class="w-full">
        <div class="w-full bg-gray-800 border-b-2 border-dotted border-gray-600 p-4 flex items-center justify-between">
            <div class="flex gap-3">
                <div class="flex gap-2 items-center">
                    ...
                    <span class="text-xl text-white">Quizzes Table</span>
                </div>
                <p class="text-xl text-white flex items-center gap-4">
                    <span>
                        <button wire:click="addQuiz" wire:loading.class="opacity-50"
            ...
            <input type="text" class="px-4 py-1 rounded-full border border-white shadow-lg"
                wire:model.live.debounce.300ms="search" placeholder="Search quiz here ...">
        </div>
        <div class="bg-white overflow-auto">
            <table class="min-w-full bg-white">
                <thead class="bg-gray-800 text-white">
                    <tr> 
                        ...
                        <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm">
                            <span class="cursor-pointer" wire:click="sort('question')">Question</span>                        
                        </th>
                        <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm">
                            <span class="cursor-pointer" wire:click="sort('answer')">Answer</span>                        
                        </th>
                        <th class="text-left py-3 px-4 uppercase font-semibold text-sm"><span class="cursor-pointer" wire:click="sort('verse')">Verse(s)</span></th>
                ...
                <tbody class="text-gray-700">
                    ...
                    <?php
                        $index = ($quizzes ->currentpage()-1) * $quizzes ->perpage() + $loop->index + 1;
                        $addClass = '';
                        if($index % 2 === 0) {
                            $addClass = 'bg-gray-200';
                        }
                    ?>
                    <tr class="{{ $addClass }}" wire:key="{{ $quiz->id }}">
                        <td class="w-5 text-left py-3 px-4 font-semibold text-sm">{{ $index }}</td>
                        <td class="w-1/3 text-left py-3 px-4">{{ $quiz->question }}</td>
                        <td class="w-1/3 text-left py-3 px-4">{{ $quiz->answer }}</td>
                        <td class="text-left py-3 px-4">{{ $quiz->verse }}</td>
                        <td class="text-left py-3 px-4">{{ $quiz->topic->name }}</td>
                        <td class="text-left py-3 px-4 flex gap-2 text-white">
                            <button wire:click="editQuiz('{{ $quiz->id }}')" type="button"
                            ...
                            <button wire:confirm="Are you sure you want to delete this quiz?"
                                wire:click="deleteQuiz({{ $quiz->id }})" type="button"
