<div class="p-5">
    @if($showForm == 'add')
    <livewire:admin.create-scavenger></livewire:admin.create-scavenger>
    @elseif($showForm == 'edit')
    <livewire:admin.update-scavenger :scavenger="$scavenger"></livewire:admin.update-scavenger>
    @else
    <div class="w-full">
        <div class="w-full bg-gray-800 border-b-2 border-dotted border-gray-600 p-4 flex items-center justify-between">
            <div class="flex gap-3">
                <div class="flex gap-2 items-center">
                    ...
                    <span class="text-xl text-white">Scavengers Table</span>
                </div>
                <p class="text-xl text-white flex items-center gap-4">                    
                    <span>
                        <button wire:click="addScavenger" wire:loading.class="opacity-50"
            ...
            <input type="text" class="px-4 py-1 rounded-full border border-white shadow-lg"
                wire:model.live.debounce.300ms="search" placeholder="Search scavenger here ...">
        </div>
            ...
            <table class="min-w-full bg-white">
                <thead class="bg-gray-800 text-white">
                    <tr>
                        ...
                        <th class="w-1/9 text-left py-3 px-4 uppercase font-semibold text-sm"><span class="cursor-pointer" wire:click="sort('page')">Question</span></th>
                        <th class="w-1/9 text-left py-3 px-4 uppercase font-semibold text-sm"><span class="cursor-pointer" wire:click="sort('title')">Answer</span></th>
                        <th class="w-1/9 text-left py-3 px-4 uppercase font-semibold text-sm"><span class="cursor-pointer" wire:click="sort('description')">Verse</span></th>     
                        <th class="w-1/9 text-left py-3 px-4 uppercase font-semibold text-sm text-center">Total Choice(s)</th>                   
                ...
                <tbody class="text-gray-700">
                    ...
                    <?php
                        $index = ($scavengers ->currentpage()-1) * $scavengers ->perpage() + $loop->index + 1;
                        $addClass = '';
                        if($index % 2 === 0) {
                            $addClass = 'bg-gray-200';
                        }
                    ?>
                    <tr class="{{ $addClass }}" wire:key="{{ $scavenger->id }}">
                        <td class="w-5 text-left py-3 px-4 font-semibold text-sm">{{ $index }}</td>
                        <td class="w-1/9 text-left py-3 px-4">{{ $scavenger->question }}</td>
                        <td class="w-1/9 text-left py-3 px-4">{{ $scavenger->answer }}</td>
                        <td class="w-1/9 text-left py-3 px-4">{{ $scavenger->verse }}</td>
                        <td class="w-1/9 text-left py-3 px-4 text-center">{{ $scavenger->targets->count() }}</td>
                        <td class="text-left py-3 px-4 flex gap-2 text-white items-center justify-center">
                            <button wire:click="editScavenger('{{ $scavenger->id }}')" type="button"
                            ...
                            <button wire:confirm="Are you sure you want to delete this scavenger?"
                                wire:click="deleteScavenger({{ $scavenger->id }})" type="button"
