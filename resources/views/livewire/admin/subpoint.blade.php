<div class="p-5">
    @if($showForm == 'add')
        <livewire:admin.create-subpoint></livewire:admin.create-subpoint>
    @elseif($showForm == 'edit')
        <livewire:admin.update-subpoint :subpoint="$subpoint"></livewire:admin.update-subpoint>
    @else
        <div class="w-full">
            <div class="w-full bg-gray-800 border-b-2 border-dotted border-gray-600 p-4 flex items-center justify-between">
                <div class="flex gap-3">
                    <div class="flex gap-2 items-center">
                        ...
                        <span class="text-xl text-white">Outline Subpoint Table</span>
                    </div>
                    <p class="text-xl text-white flex items-center gap-4">
                        <span>
                            <button wire:click="addSubpoint" wire:loading.class="opacity-50"
                ...
                <input type="text" class="px-4 py-1 rounded-full border border-white shadow-lg"
                    wire:model.live.debounce.300ms="search" placeholder="Search subpoint here ...">
            </div>
            <div class="bg-white overflow-auto">
                <table class="min-w-full bg-white">
                    <thead class="bg-gray-800 text-white">
                        <tr> 
                            ...
                            <th class="w-1/5 text-left py-3 px-4 uppercase font-semibold text-sm">
                                <span class="cursor-pointer" wire:click="sort('sub')">Sub Point</span>          
                            </th>
                            ...
                            <th class="w-1/5 text-left py-3 px-4 uppercase font-semibold text-sm">
                                <span class="cursor-pointer" wire:click="sort('body')">Body</span>     
                            </th>
                            <th class="w-1/5 text-left py-3 px-4 uppercase font-semibold text-sm">Main Point</th>
                            <th class="w-1/5 text-left py-3 px-4 uppercase font-semibold text-sm">Theme</th>
                    ...
                    <tbody class="text-gray-700">
                        ...
                        <?php
                            $index = ($subpoints ->currentpage()-1) * $subpoints ->perpage() + $loop->index + 1;
                            $addClass = '';
                            if($index % 2 === 0) {
                        ...
                        <tr class="{{ $addClass }}" wire:key="{{ $point->id }}">
                            ...
                            <td class="text-left py-3 px-4">{{ $point->sub }}</td>
                            ...
                            <td class="text-left py-3 px-4 flex gap-2 text-white">                             
                                ...
                                <button wire:confirm="Are you sure you want to delete this main point?"
                                    wire:click="deleteSubpoint({{ $point->id }})" type="button"
