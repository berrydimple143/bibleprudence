<div class="p-5">
    @if($showForm == 'add')
    <livewire:admin.create-target></livewire:admin.create-target>
    @elseif($showForm == 'edit')
    <livewire:admin.update-target :target="$target"></livewire:admin.update-target>
    @else
    <div class="w-full">
        <div class="w-full bg-gray-800 border-b-2 border-dotted border-gray-600 p-4 flex items-center justify-between">
            <div class="flex gap-3">
                <div class="flex gap-2 items-center">
                    ...
                    <span class="text-xl text-white">Scavenger Choices Table</span>
                </div>
                <p class="text-xl text-white flex items-center gap-4">
                    <span>
                        <button wire:click="addTarget" wire:loading.class="opacity-50"
            ...
            <table class="min-w-full bg-white">
                <thead class="bg-gray-800 text-white">
                    <tr>
                        ...
                        <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm">Scavenger Question</th>
                ...
                <tbody class="text-gray-700">
                    ...
                    <?php
                        $index = ($targets ->currentpage()-1) * $targets ->perpage() + $loop->index + 1;
                        $addClass = '';
                        if($index % 2 === 0) {
                            $addClass = 'bg-gray-200';
                        }
                    ?>
                    <tr class="{{ $addClass }}" wire:key="{{ $target->id }}">
                        <td class="w-5 text-left py-3 px-4 font-semibold text-sm">{{ $index }}</td>
                        <td class="w-1/3 text-left py-3 px-4">{{ $target->scavenger->question }} in {{ $target->scavenger->verse }}</td>
                        <td class="w-1/3 text-left py-3 px-4">{{ $target->option }}</td>
                        <td class="text-left py-3 px-4 flex gap-2 text-white">
                            <button wire:click="editTarget('{{ $target->id }}')" type="button"
                            ...
                            <button wire:confirm="Are you sure you want to delete this target?"
                                wire:click="deleteTarget({{ $target->id }})" type="button"
