<div class="p-5">
    @if($showForm == 'add')
    <livewire:admin.create-topic></livewire:admin.create-topic>
    @elseif($showForm == 'edit')
    <livewire:admin.update-topic :topic="$topic"></livewire:admin.update-topic>
    @else
    <div class="w-full">
        <div class="w-full bg-gray-800 border-b-2 border-dotted border-gray-600 p-4 flex items-center justify-between">
            <div class="flex gap-3">
                <div class="flex gap-2 items-center">
                    ...
                    <span class="text-xl text-white">Topics Table</span>
                </div>
                <p class="text-xl text-white flex items-center gap-4">                    
                    <span>
                        <button wire:click="addTopic" wire:loading.class="opacity-50"
            ...
            <input type="text" class="px-4 py-1 rounded-full border border-white shadow-lg"
                wire:model.live.debounce.300ms="search" placeholder="Search topic here ...">
        </div>
            ...
            <table class="min-w-full bg-white">
                <thead class="bg-gray-800 text-white">
                    <tr>
                        ...
                        <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm"><span class="cursor-pointer" wire:click="sort('name')">Topic Name</span></th>
                        <th class="w-1/3 text-left py-3 px-4 uppercase font-semibold text-sm">Number of Quizzes</th>
                ...
                <tbody class="text-gray-700">
                    ...
                    <?php
                        $index = ($topics ->currentpage()-1) * $topics ->perpage() + $loop->index + 1;
                        $addClass = '';
                        if($index % 2 === 0) {
                            $addClass = 'bg-gray-200';
                        }
                    ?>
                    <tr class="{{ $addClass }}" wire:key="{{ $topic->id }}">
                        <td class="w-5 text-left py-3 px-4 font-semibold text-sm">{{ $index }}</td>
                        <td class="w-1/3 text-left py-3 px-4">{{ $topic->name }}</td>
                        <td class="w-1/3 text-left py-3 px-4">{{ $topic->quizzes->count() }}</td>
                        <td class="text-left py-3 px-4 flex gap-2 text-white">
                            <button wire:click="editTopic('{{ $topic->id }}')" type="button"
                            ...
                            <button wire:confirm="Are you sure you want to delete this topic?"
                                wire:click="deleteTopic({{ $topic->id }})" type="button"
